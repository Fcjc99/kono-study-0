...
import { uid, localDate, type AppData } from '../store/model'
import { parseFlashcards, reviewQueue, fullDeckQueue, nextDueIn, type FlashcardDeck } from '../store/flashcards'
import { srsInitial, srsGrade, isDue } from '../store/spacedRepetition'
...
export default function Flashcards({ profileId, decks, setData, draftKey='kono-card-draft' }: { draftKey?:string; profileId: string; decks: FlashcardDeck[]; setData: (action: SetStateAction<AppData>) => Promise<boolean> }) {
  ...
  const review = (event: FormEvent) => {
    ...
    try { if (!title.trim()) throw new Error('Give this deck a name.'); importGeneration.current++; setPreview({ id: editing?.id??uid('deck'), profileId, title: title.trim(), cards: parseFlashcards(text, () => uid('card'), today).map((c,i)=>editing?.cards[i]?{...c,id:editing.cards[i].id,...srsInitial(today),reviewId:uid('review')}:c) }) }
    catch (error) { setMessage(error instanceof Error ? error.message : 'Check the question-and-answer format.') }
  }
  const startQuiz = (d: FlashcardDeck, mode: 'due' | 'missed' | 'all' = 'due') => { setSession({ deckId: d.id, ids: mode === 'all' ? fullDeckQueue(d) : reviewQueue(d, today, mode === 'missed'), index: 0 }); setRevealed(false); setMessage('') }
  const grade = (correct: boolean) => {
    if (!deck || !card || !session) return
    const expected = card, active = session, reviewId = uid('review'), srs = srsGrade(card, correct, today)
    void run(() => setData(data => {
      ...
      return { ...data, flashcardDecks: data.flashcardDecks.map(d => d.id === deck.id ? { ...d, cards: d.cards.map(c => c.id === card.id ? { ...c, ...srs, reviewId } : c) } : d) }
    }), () => { setSession({ ...active, index: active.index + 1 }); setRevealed(false) })
  }
  return <section className="card study-planner flashcards"><div className="card-head"><div><span className="eyebrow">Quiz me</span><h3>Your flashcards</h3><p>Write your own questions or import a prepared question-and-answer list. No AI needed. Cards you miss come back sooner; cards you know well space further apart.</p></div><button type="button" className="primary" onClick={() => { setEditing(null);setOpen(true); setMessage('') }}>＋ Create / import cards</button></div>
    ...
    {open && !preview && <form className="study-plan-form" onSubmit={review}><fieldset disabled={busy}><label>Deck name<input required maxLength={200} value={title} onChange={e => setTitle(e.target.value)} placeholder="Biology — key terms" /></label><label>Question-and-answer list<textarea required rows={7} maxLength={100000} value={text} onChange={e => { importGeneration.current++; setText(e.target.value) }} placeholder={'What is photosynthesis? :: Turning light into chemical energy\nWhat carries genetic information? :: DNA'} /></label><p>One card per line: question <strong> :: </strong> answer. Tab-separated pairs also work. Up to 200 cards. Ordinary notes and PDFs need to be turned into pairs first.</p><label>Import a text or TSV file<input type="file" accept=".txt,.tsv,text/plain,text/tab-separated-values" onChange={e => {
      ...
      void file.text().then(value => { if (generation !== importGeneration.current) return; parseFlashcards(value, () => uid('check'), today); setText(value); setMessage('Imported into the draft. Preview before saving.') }).catch(error => { if (generation === importGeneration.current) setMessage(error instanceof Error ? error.message : 'Could not read this file.') })
    ...
    {session && <div className="flashcard-practice" aria-live="polite">{!deck ? <p>This deck is no longer available.</p> : session.ids.length === 0 ? <>{(() => { const days = nextDueIn(deck, today); return <p>Nothing’s due for review right now{days !== null ? ` — the next card comes due in ${days} day${days === 1 ? '' : 's'}.` : '.'}</p> })()}<button type="button" className="primary" onClick={() => startQuiz(deck, 'all')}>Study anyway</button></> : session.index >= session.ids.length ? <><h4>Round finished</h4><p>{deck.cards.filter(c => c.interval === 0 && isDue(c, today)).length} cards still due for review.</p><button type="button" className="primary" disabled={!deck.cards.some(c => c.interval === 0 && isDue(c, today))} onClick={() => startQuiz(deck, 'missed')}>Repeat missed cards</button></> : card ? <><p>{deck.title} · Card {session.index + 1} of {session.ids.length}</p><h4>{card.question}</h4>{!revealed ? <button type="button" className="primary" onClick={() => setRevealed(true)}>Show answer</button> : <><p className="flashcard-answer">{card.answer}</p><div className="study-actions"><button type="button" className="primary" disabled={busy} onClick={() => grade(true)}>Got it</button><button type="button" className="secondary" disabled={busy} onClick={() => grade(false)}>Review again</button></div></>}</> : <p>This card was removed. Start a new round.</p>}<button type="button" className="secondary" disabled={busy} onClick={() => setSession(null)}>Close quiz</button></div>}
    <div className="flashcard-decks">{decks.map(d => <article key={d.id}><div><h4>{d.title}</h4><p>{d.cards.length} cards · {d.cards.filter(c => isDue(c, today)).length} due now</p></div><div className="study-actions"><button type="button" className="primary" disabled={busy} onClick={() => startQuiz(d)}>Quiz me</button><button type="button" disabled={busy} onClick={()=>{setEditing(d);setTitle(d.title);setText(d.cards.map(c=>c.question+' :: '+c.answer).join('\n'));setOpen(true);setPreview(null);setSession(null)}}>Edit cards</button><button type="button" disabled={busy} onClick={()=>void run(()=>setData(data=>({...data,flashcardDecks:[...data.flashcardDecks,{...d,id:uid('deck'),title:d.title+' (copy)',cards:d.cards.map(c=>({...c,id:uid('card'),...srsInitial(today)}))}]})),()=>setMessage('Deck duplicated.'))}>Duplicate</button><button type="button" className="secondary" disabled={busy} onClick={() => { setDeleting(d.id) }}>Delete deck</button></div></article>)}</div>
  </section>
}
