...
export default function UpdatePlanScanner({ profileId, subjects, kids, save, close }: { profileId: string; subjects: Subject[]; kids: Kid[]; save: (fn: (d: AppData) => AppData) => Promise<boolean>; close?: () => void }) {
  ...
  const confirmItem = async (item: CreatedItem) => {
    const ok = await save(d => {
      ...
      return { ...d, calendarEvents: d.calendarEvents.map(e => e.id === item.id ? { ...e, title: item.title, date: item.date, time: item.time, endTime: item.endTime, subjectId: item.subjectId, kidId: item.kidId, needsReview: false } : e) }
    })
    ...
    {createdItems.length > 0 && <div className="update-plan-results">
      ...
      {createdItems.map(item => <div className="update-plan-result-row" key={item.id}>
        ...
        <div className="update-plan-result-fields">
          ...
          {item.key === 'calendarEvents' && <label>Start time (optional)<input type="time" value={item.time ?? ''} onChange={e => editRow(item.id, { time: e.target.value || undefined })} /></label>}
          {item.key === 'calendarEvents' && <label>End time (optional)<input type="time" value={item.endTime ?? ''} onChange={e => editRow(item.id, { endTime: e.target.value || undefined })} /></label>}
