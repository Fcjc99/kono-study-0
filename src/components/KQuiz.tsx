...
import { buildKQuizSetPdf } from '../store/kquizPdf'
...
export default function KQuiz({ profileId, lectures, sets, sources, decks, subjects, setData, onOpenAiSettings }: { profileId: string; onOpenAiSettings?: () => void; lectures: KQuizLecture[]; sets: KQuizSet[]; sources: KQuizSource[]; decks: FlashcardDeck[]; subjects: Subject[]; setData: (action: SetStateAction<AppData>) => Promise<boolean> }) {
  ...
  const downloadSetPdf = async (set: KQuizSet) => {
    try {
      const subjectName = subjects.find(s => s.id === set.subjectId)?.name ?? ''
      const blob = await buildKQuizSetPdf(set, subjectName)
      downloadBlob(blob, set.title.replace(/[^\w\- ]+/g, '').trim().slice(0, 80) + '.pdf')
      setMessage('Downloaded as PDF.')
    } catch { setMessage('Could not create the PDF. Try again.') }
  }
  ...
  const askQuestion = async () => {
    if (needsApiKey()) return
    if (!qaQuestion.trim()) { setMessage('Type a question first.'); return }
    ...
    {checkedCount > 0 && <div className="wb-panel kquiz-combine">
      ...
      <form className="study-plan-form" onSubmit={e => { e.preventDefault(); void generateFromChecked() }}><fieldset disabled={busy || generatingFor === 'combined'}>
        ...
        <div className="study-actions">
          ...
          <button type="button" className="secondary" onClick={() => { setChecked(new Set()); setQaQuestion(''); setQaAnswer(null) }}>Clear selection</button>
        </div>
      </fieldset></form>
      <form className="study-plan-form kquiz-ask-form" onSubmit={e => { e.preventDefault(); void askQuestion() }}><fieldset disabled={busy || qaBusy}>
        <label>Or just ask a question about {checkedCount === 1 ? 'it' : 'these'}<input required maxLength={500} value={qaQuestion} onChange={e => setQaQuestion(e.target.value)} placeholder="What's the difference between mitosis and meiosis?" /></label>
        <div className="study-actions"><button className="primary">{qaBusy ? 'Thinking…' : 'Ask'}</button></div>
      </fieldset></form>
      {qaAnswer && <div className="kquiz-guide-panel"><KQuizGuide text={qaAnswer} /><p className="kquiz-ai-note">This answer was generated by AI and may contain mistakes — check it against the original material.</p></div>}
    </div>}
        ...
        return <article key={set.id} className="kquiz-set">
          ...
          {(set.summary || set.studyGuide) && <>
            ...
            <div className="kquiz-guide-panel">
              ...
              <div className="study-actions"><button type="button" className="secondary" onClick={() => void downloadSetPdf(set)}>⬇ Download as PDF</button></div>
            </div>
          </>}
