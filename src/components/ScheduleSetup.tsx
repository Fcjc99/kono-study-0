...
function ManualWeekly({data,save,draftKey}:{data:AppData;save:PlannerRepository['update'];draftKey:string}){
 ...
 return <form className="wb-panel school-setup" onSubmit={e=>{e.preventDefault();void submit()}}><fieldset disabled={busy}><h3>Add work or a weekly activity</h3><p>For example: name it Work, add Monday 8 AM–4 PM, then Tuesday 9 AM–3 PM. Select several days together when their hours match. Each date has its own notes and completion controls.</p><label>Link a subject (optional)<select value={input.subjectId} onChange={e=>{const subject=data.subjects.find(s=>s.id===e.target.value);change({subjectId:e.target.value,title:subject?.name??input.title})}}><option value="">No subject — work or personal activity</option>{data.subjects.filter(s=>s.profileId===data.activeProfileId).map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></label>{data.kids.some(k=>k.profileId===data.activeProfileId)&&<label>Kid (optional)<select value={input.kidId??''} onChange={e=>change({kidId:e.target.value})}><option value="">Whole family / unassigned</option>{data.kids.filter(k=>k.profileId===data.activeProfileId).map(k=><option key={k.id} value={k.id}>{(k.emoji?k.emoji+' ':'')+k.name}</option>)}</select></label>}<label>Schedule name<input placeholder="Work, Gym, Volunteering…" required maxLength={200} value={input.title} onChange={e=>change({title:e.target.value})}/></label>
