...
function SportsGames({data,save,draftKey,sport}:{data:AppData;save:PlannerRepository['update'];draftKey:string;sport:string}){
 ...
 const suggestions=()=>{setRows(suggestSchedule(text,start,end,order).map(r=>r.kind==='class'?r:({...r,kind:'event' as const})));setReviewed(false);setStatus('Review every suggestion. Unrecognized lines stay in the source text; add missing games manually.')}
 const commit=async()=>{
 ...
 {rows.length>0&&<><h4>Review games</h4><p>Check each game against the original. Only selected items will be added, tagged as sports.</p>{rows.map((row,i)=><fieldset className="import-row" key={row.id}><legend>Item {i+1}</legend><label className="import-select"><input type="checkbox" checked={row.include} onChange={e=>update(row.id,{include:e.target.checked})}/> Add this item</label><details><summary>Source text</summary><p>{row.source}</p></details><div className="import-fields"><label>Title<input maxLength={200} value={row.title} onChange={e=>update(row.id,{title:e.target.value})}/></label><label>Date<input type="date" value={row.date} onInput={e=>update(row.id,{date:e.currentTarget.value})}/></label></div><div className="import-fields"><label>Start time<input type="time" value={row.start} onInput={e=>update(row.id,{start:e.currentTarget.value})}/></label><label>End time<input type="time" value={row.end} onInput={e=>update(row.id,{end:e.currentTarget.value})}/></label></div><button onClick={()=>{setRows(rows.filter(r=>r.id!==row.id));setReviewed(false)}}>Remove suggestion</button></fieldset>)}
