...
export default function Sidebar({view,onView,profile='Sophia',schoolYear='Summer 2026',profiles=[],activeProfileId,onProfileChange,weekWeather=[],weatherLocation,music,parentMode=false,weatherChip}:{weatherChip?:ReactNode;view:ViewName;onView:(v:ViewName)=>void;profile?:string;schoolYear?:string;profiles?:ProfileOption[];activeProfileId?:string;onProfileChange?:(id:string)=>void;weekWeather?:DailyWeatherForecast[];weatherLocation?:string;music?:ReturnType<typeof useMusicController>;parentMode?:boolean}){
 ...
 return <><aside className="sidebar"><div className="brand"><div className="brand-mark" aria-hidden="true"><span>K</span><i>✿</i></div><div><div className="brand-title-row"><h1 role="none">KONO</h1>{weatherChip}</div><p>Study Sanctuary</p></div></div><div className="sidebar-section-label">Notebook tabs</div><nav className="nav-list" aria-label="Notebook tabs">{items.map(item=><button className={`nav-button ${view===item?'active':''}`} key={item} onClick={()=>onView(item)}><span className="nav-icon"><NavIcon name={item}/></span><span>{item}</span><i className="nav-petal" aria-hidden="true">✦</i></button>)}</nav>{music&&<div className="sidebar-music-slot"><MusicPlayer controller={music} compact/></div>}<section className="season-card profile-dock"><div className="profile-avatar">{profile.slice(0,1).toUpperCase()}</div><div className="profile-dock-copy"><span>{profile}</span><strong>{schoolYear}</strong><small>Active study profile</small></div>{profiles.length>0&&<select aria-label="Switch study profile" value={activeProfileId} onChange={e=>onProfileChange?.(e.target.value)}>{profiles.map(p=><option key={p.id} value={p.id}>{p.label}</option>)}</select>}</section></aside><header className="mobile-desk-header"><div className="mobile-brand-row"><div className="mobile-brand-lockup"><div className="brand-mark" aria-hidden="true"><span>K</span><i>✿</i></div><div><div className="brand-title-row"><h1 role="none">KONO</h1>{weatherChip}</div><p>Study Sanctuary</p></div></div><div className="mobile-profile-chip"><span>{profile}</span><small>{schoolYear}</small>{profiles.length>0&&<select aria-label="Switch study profile" value={activeProfileId} onChange={e=>onProfileChange?.(e.target.value)}>{profiles.map(p=><option key={p.id} value={p.id}>{p.label}</option>)}</select>}</div></div><div className="mobile-tabs-label">Notebook tabs</div><nav className="mobile-nav" aria-label="Pages" style={{'--nav-cols':items.length} as CSSProperties}>{items.map(item=><button className={view===item?'active':''} key={item} onClick={()=>onView(item)}><span><NavIcon name={item}/></span><small>{item}</small><i aria-hidden="true">✦</i></button>)}</nav><WeekWeather days={weekWeather} location={weatherLocation}/></header></>
}
