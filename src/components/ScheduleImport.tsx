...
export default function ScheduleImport({data,save}:{data:AppData;save:PlannerRepository['update']}){
 ...
 <details><summary>AI settings</summary><label>Provider<select value={photoProvider} onChange={e=>setPhotoProvider(e.target.value)}><option value="gemini">Google Gemini (free tier available)</option><option value="openai">OpenAI</option></select></label><label>API key<input type="password" autoComplete="off" value={photoApiKey} onChange={e=>setPhotoApiKey(e.target.value)} placeholder="Paste your API key"/></label></details>
