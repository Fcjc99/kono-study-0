...
export class PlannerRepository {
 ...
 fetchSchoolCatalog=async()=>{if(!this.cloud)throw new Error('The shared catalog needs a KONO deployment with cloud storage configured.');return this.cloud.fetchSchoolCatalog()}
 submitSchoolCatalogEntry=async(entry:Parameters<SupabaseRemote['submitSchoolCatalogEntry']>[0])=>{if(!this.cloud)throw new Error('The shared catalog needs a KONO deployment with cloud storage configured.');return this.cloud.submitSchoolCatalogEntry(entry)}
 ...
 setUsername=async(username:string,displayName:string)=>this.requireCloud().setUsername(username,displayName)
 findClassmates=async(query:string)=>this.requireCloud().findClassmates(query)
 sendConnectionRequest=async(recipientId:string)=>this.requireCloud().sendConnectionRequest(recipientId)
 respondToConnection=async(id:string,accept:boolean)=>this.requireCloud().respondToConnection(id,accept)
 removeConnection=async(id:string)=>this.requireCloud().removeConnection(id)
