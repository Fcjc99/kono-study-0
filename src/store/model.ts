...
export type KidBorderStyle='cute'|'sports'|'modern'|'unique'
...
export type CalendarEvent={id:string;profileId:string;date:string;title:string;kind:CalendarEventKind;done?:boolean;subjectId?:string;kidId?:string;notes:string;time?:string;endTime?:string;result?:MatchResult;needsReview?:boolean}
...
export type SettingsData={experience?:'cozy'|'simplified'|'modern'|'sumi';theme:string;themeVersion:number;sound:boolean;ambient:boolean;reminders:boolean;browserNotifications?:boolean;familyEventReminders?:boolean;familyTileBorders?:boolean;familyTileAvatars?:boolean;loginDigest?:boolean;scheduleShowAcademic?:boolean;scheduleShowSports?:boolean;scheduleShowAppointments?:boolean;parentMode?:boolean;reducedMotion:boolean;textSize?:'normal'|'large';density?:'comfortable'|'compact';decoration?:boolean;boardStyle?:'paper'|'cork'|'plain';motionPreference?:'system'|'reduced'|'full';sanctuaryWeather:SanctuaryWeather;sanctuaryWeatherMode:WeatherMode;sanctuaryZipCodes:Record<string,string>;sanctuaryWeatherLocations:Record<string,string>}
...
export function normalizeData(raw:unknown):AppData {
 ...
 // A save from before the border-style redesign carries one of the old six values (solid was the
 // default, so this covers essentially every existing kid) -- map it onto its closest new style
 // rather than rejecting the whole save over a cosmetic field.
 ...
 const kids:Kid[]=list(raw.kids??[],'kids',500).map(k=>({id:id(k.id,'kid ID'),profileId:owner(k.profileId),name:str(k.name,'kid name',200),color:color(k.color,'#7ca982'),emoji:optional(k.emoji,'kid emoji',40),borderStyle:choice(typeof k.borderStyle==='string'?legacyBorderStyle[k.borderStyle]??k.borderStyle:k.borderStyle,['cute','sports','modern','unique'],'modern')}))
 ...
 const calendarEvents:CalendarEvent[]=eventInput.map(e=>{const profileId=owner(e.profileId);return {id:id(e.id,'event ID'),profileId,done:bool(e.done),subjectId:subject(e.subjectId,profileId),kidId:kidRef(e.kidId,profileId),title:str(e.title,'event title',1000),date:date(e.date,'event date'),notes:str(e.notes??'','event notes',100000),kind:choice(e.kind,['exam','test','quiz','assignment','study','activity','personal','sports','appointment','work','other'],'other'),time:e.time===undefined?undefined:clockTime(e.time),endTime:e.endTime===undefined?undefined:clockTime(e.endTime),result:matchResult(e.result),needsReview:bool(e.needsReview)}})
 ...
 const settings:SettingsData={...defaultSettings,experience:choice(experienceRaw,['cozy','simplified','modern','sumi'],s.theme==='professional'?'modern':'cozy'),textSize:choice(s.textSize,['normal','large'],'normal'),density:choice(s.density,['comfortable','compact'],'comfortable'),decoration:bool(s.decoration,true),boardStyle:choice(s.boardStyle,['paper','cork','plain'],'paper'),theme:choice(s.theme,['coral','sakura','lavender','mint','honey','zen','floral','professional','forest','ocean','midnight','paper'],'coral'),sound:bool(s.sound,true),ambient:bool(s.ambient,true),reminders:bool(s.reminders),browserNotifications:bool(s.browserNotifications),familyEventReminders:bool(s.familyEventReminders),familyTileBorders:bool(s.familyTileBorders,true),familyTileAvatars:bool(s.familyTileAvatars,true),loginDigest:bool(s.loginDigest,true),scheduleShowAcademic:bool(s.scheduleShowAcademic,legacyView!=='sports'),scheduleShowSports:bool(s.scheduleShowSports,legacyView!=='academic'),scheduleShowAppointments:bool(s.scheduleShowAppointments,true),parentMode:bool(s.parentMode,kids.length>0),reducedMotion:bool(s.reducedMotion),motionPreference:choice(s.motionPreference,['system','reduced','full'],s.reducedMotion===true?'reduced':'system'),sanctuaryWeather:choice(s.sanctuaryWeather,['clear','cloudy','rain','wind','snow'],'clear'),sanctuaryWeatherMode:choice(s.sanctuaryWeatherMode,['live','manual','clear'],'clear'),sanctuaryZipCodes:stringMap(s.sanctuaryZipCodes),sanctuaryWeatherLocations:stringMap(s.sanctuaryWeatherLocations??s.sanctuaryZipCodes)}
