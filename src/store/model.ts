...
export type CalendarEventKind='exam'|'test'|'quiz'|'assignment'|'study'|'activity'|'personal'|'sports'|'other'
...
export type SettingsData={experience?:'cozy'|'simplified'|'office';theme:string;themeVersion:number;sound:boolean;ambient:boolean;reminders:boolean;browserNotifications?:boolean;loginDigest?:boolean;scheduleView?:'all'|'academic'|'sports';reducedMotion:boolean;textSize?:'normal'|'large';density?:'comfortable'|'compact';decoration?:boolean;boardStyle?:'paper'|'cork'|'plain';motionPreference?:'system'|'reduced'|'full';sanctuaryWeather:SanctuaryWeather;sanctuaryWeatherMode:WeatherMode;sanctuaryZipCodes:Record<string,string>;sanctuaryWeatherLocations:Record<string,string>}
...
export function normalizeData(raw:unknown):AppData {
 ...
 const settings:SettingsData={...defaultSettings,experience:choice(s.experience,['cozy','simplified','office'],s.theme==='professional'?'office':'cozy'),textSize:choice(s.textSize,['normal','large'],'normal'),density:choice(s.density,['comfortable','compact'],'comfortable'),decoration:bool(s.decoration,true),boardStyle:choice(s.boardStyle,['paper','cork','plain'],'paper'),theme:choice(s.theme,['coral','sakura','lavender','mint','honey','professional','forest','ocean','midnight','paper'],'coral'),sound:bool(s.sound,true),ambient:bool(s.ambient,true),reminders:bool(s.reminders),browserNotifications:bool(s.browserNotifications),loginDigest:bool(s.loginDigest,true),scheduleView:choice(s.scheduleView,['all','academic','sports'],'all'),reducedMotion:bool(s.reducedMotion),motionPreference:choice(s.motionPreference,['system','reduced','full'],s.reducedMotion===true?'reduced':'system'),sanctuaryWeather:choice(s.sanctuaryWeather,['clear','cloudy','rain','wind','snow'],'clear'),sanctuaryWeatherMode:choice(s.sanctuaryWeatherMode,['live','manual','clear'],'clear'),sanctuaryZipCodes:stringMap(s.sanctuaryZipCodes),sanctuaryWeatherLocations:stringMap(s.sanctuaryWeatherLocations??s.sanctuaryZipCodes)}
