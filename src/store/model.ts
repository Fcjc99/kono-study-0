...
export type Task={id:string;profileId:string;subjectId:string;title:string;due:string;done:boolean;notes:string;completedAt?:string;studyPlanId?:string;unitNumber?:number}
...
export type CalendarEventKind='exam'|'test'|'quiz'|'assignment'|'study'|'activity'|'personal'|'other'
export type CalendarEvent={id:string;profileId:string;date:string;title:string;kind:CalendarEventKind;done?:boolean;subjectId?:string;notes:string}
export type SettingsData={experience?:'cozy'|'simplified'|'office';theme:string;themeVersion:number;sound:boolean;ambient:boolean;reminders:boolean;reducedMotion:boolean;textSize?:'normal'|'large';density?:'comfortable'|'compact';decoration?:boolean;boardStyle?:'paper'|'cork'|'plain';motionPreference?:'system'|'reduced'|'full';sanctuaryWeather:SanctuaryWeather;sanctuaryWeatherMode:WeatherMode;sanctuaryZipCodes:Record<string,string>;sanctuaryWeatherLocations:Record<string,string>}
...
export type StudySeason={school?:SchoolCalendar;id:string;profileId:string;name:string;start:string;end:string;active:boolean;week:WeekSchedule}
...
export const defaultSettings:SettingsData={experience:'cozy',textSize:'normal',density:'comfortable',decoration:true,boardStyle:'paper',theme:'coral',themeVersion:4,sound:true,ambient:true,reminders:false,reducedMotion:false,motionPreference:'system',sanctuaryWeather:'clear',sanctuaryWeatherMode:'clear',sanctuaryZipCodes:{},sanctuaryWeatherLocations:{}}
...
export function normalizeData(raw:unknown):AppData {
 ...
 const studySeasons:StudySeason[]=list(raw.studySeasons??[],'schedules',500).map(s=>{
  ...
  return {school,id:id(s.id,'season ID'),profileId,name:str(s.name,'season name',200),start:date(s.start,'season start'),end:date(s.end,'season end'),active:bool(s.active),week}
 })
 ...
 const settings:SettingsData={...defaultSettings,experience:choice(s.experience,['cozy','simplified','office'],s.theme==='professional'?'office':'cozy'),textSize:choice(s.textSize,['normal','large'],'normal'),density:choice(s.density,['comfortable','compact'],'comfortable'),decoration:bool(s.decoration,true),boardStyle:choice(s.boardStyle,['paper','cork','plain'],'paper'),theme:choice(s.theme,['coral','sakura','lavender','mint','honey','professional','forest','ocean','midnight','paper'],'coral'),sound:bool(s.sound,true),ambient:bool(s.ambient,true),reminders:bool(s.reminders),reducedMotion:bool(s.reducedMotion),motionPreference:choice(s.motionPreference,['system','reduced','full'],s.reducedMotion===true?'reduced':'system'),sanctuaryWeather:choice(s.sanctuaryWeather,['clear','cloudy','rain','wind','snow'],'clear'),sanctuaryWeatherMode:choice(s.sanctuaryWeatherMode,['live','manual','clear'],'clear'),sanctuaryZipCodes:stringMap(s.sanctuaryZipCodes),sanctuaryWeatherLocations:stringMap(s.sanctuaryWeatherLocations??s.sanctuaryZipCodes)}
