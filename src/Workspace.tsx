import {useEffect,useRef,useState,type CSSProperties,type DragEvent,type FormEvent,type ReactNode,type TouchEvent} from 'react'
...
function Workspace({store}:{store:Store}){
 ...
 const [islandZoom,setIslandZoom]=useState(1)
 const MIN_ISLAND_ZOOM=1,MAX_ISLAND_ZOOM=2.5
 // Two-finger pinch as the touch-native way to zoom the island, alongside the +/- buttons (kept for
 // mouse/keyboard/screen-reader users). Only tracks while exactly two touches are down; a third
 // finger or a drop to one/zero touches just clears the gesture rather than fighting it.
 ...
 const onIslandTouchStart=(e:TouchEvent<HTMLDivElement>)=>{if(islandMode==='view'&&e.touches.length===2)pinchStart.current={distance:touchDistance(e.touches[0],e.touches[1]),zoom:islandZoom}}
 const onIslandTouchMove=(e:TouchEvent<HTMLDivElement>)=>{
  if(!pinchStart.current||e.touches.length!==2)return
  e.preventDefault()
  const ratio=touchDistance(e.touches[0],e.touches[1])/pinchStart.current.distance
  setIslandZoom(Math.min(MAX_ISLAND_ZOOM,Math.max(MIN_ISLAND_ZOOM,+(pinchStart.current.zoom*ratio).toFixed(2))))
 }
 const onIslandTouchEnd=(e:TouchEvent<HTMLDivElement>)=>{if(e.touches.length<2)pinchStart.current=null}
 ...
 return <div className={'workbench density-'+(data.settings.density??'comfortable')+' text-'+(data.settings.textSize??'normal')} data-page={page.toLowerCase()} data-experience={experience} data-decoration={data.settings.decoration!==false}>
  ...
  <main id="workspace-main"><header className="wb-header"><div><small>{profile.label}</small><h1>{page}{experience==='cozy'&&<span className="cozy-heading-flower" aria-hidden="true">❀</span>}</h1></div><div className="wb-toolbar"><button onClick={()=>setSearch(true)}>{experience==='cozy'&&<ActionIcon name="search"/>}Search</button>{experience==='cozy'&&<button onClick={()=>navigate('Trash')}><ActionIcon name="trash"/>Trash</button>}<button className="primary" onClick={()=>setAdding(true)}>＋ Add</button>{voiceSupported&&<button className={'voice-add-button'+(voiceListening?' is-listening':'')} onClick={toggleVoiceAdd} aria-pressed={voiceListening} aria-label={voiceListening?'Stop voice input':'Add by voice'} title="Speak an assignment, exam, note or appointment — it opens in the right editor to review"><span className="kono-mic-face" aria-hidden="true"><img src={voiceListening?'/garden/kono/excited.png':'/garden/kono/idle.png'} alt=""/></span>{voiceListening?'Listening…':''}</button>}</div>{konoPhrase&&<div className="kono-speech-bubble" role="status"><img src="/garden/kono/idle.png" alt="" aria-hidden="true"/><p>{konoPhrase.text}</p><button type="button" aria-label="Dismiss" onClick={()=>setKonoPhrase(null)}>×</button></div>}{voiceError&&<p role="alert" className="voice-add-error">{voiceError}</p>}</header>
   ...
   {page==='Sanctuary'&&<>
    ...
    <details className="wb-panel sanctuary-music" open><summary>Music</summary><MusicPlayer controller={music}/></details>
...
function RecordCard({cozy,collection,entry,subject,subjectColor,seriesCount=0,compact=false,edit,remove,removeSeries,duplicate,toggle,pin,remind}:{cozy:boolean;collection:Collection;entry:Entry;subject?:string;subjectColor?:string;seriesCount?:number;compact?:boolean;edit:()=>void;remove:()=>void;removeSeries?:()=>void;duplicate:()=>void;toggle:()=>void;pin:()=>void;remind:()=>void}){
 ...
 return <article className={'wb-record wb-record-'+collection+' '+(paper?'wb-sticky note-font-'+String(entry.font??'rounded'):'')+(done?' is-complete':'')} style={paper?{'--note-paper':String(entry.color??'#fff2b4'),'--note-ink':String(entry.textColor??'#2f2942')} as CSSProperties:{'--subject-accent':subjectColor??String(entry.color??'#b47e75')} as CSSProperties}>
  <>{paper&&<span className="push-pin pin-0" aria-hidden="true"/>}</><small>{subject??(entry.subjectId?'Removed subject':'General')} · {labels[collection]}{note&&entry.pinned?' · Pinned':''}</small><h3>{titleOf(entry)}</h3>{Boolean(entry.due||entry.date)&&<p>{dateLabel(String(entry.due??entry.date))}{collection==='calendarEvents'&&entry.time?' · '+classTime(String(entry.time)):''}{done?' · Completed':''} {due&&<span className={'countdown-chip '+(daysUntil(due)<0?'is-overdue':'')}>{dueLabel}</span>}{subtaskProgress&&<span className="subtask-chip">{subtaskProgress}</span>}{inSeries&&<span className="subtask-chip">1 of {seriesCount} dates</span>}</p>}
...
function EntryEditor({edit,data,save,draftScope,close}:{edit:Edit;data:AppData;save:Store['repository']['update'];draftScope:string;close:(saved:boolean,discarded?:boolean)=>void}){
 ...
 const submit=async(e:FormEvent)=>{
  ...
  const payload={...entry,[titleKey]:String(entry[titleKey]??'').trim(),...(edit.key==='notes'&&!edit.original?{pinned:true}:{}),...(recurringId?{recurringId}:{})}
