...
import {classOccurrences,classTime,timeRange,type ClassOccurrence} from './store/classSchedule'
...
function RecordCard({cozy,collection,entry,subject,subjectColor,seriesCount=0,compact=false,edit,remove,removeSeries,duplicate,toggle,pin,remind,scheduleReview,confirmReview,startFocus}:{cozy:boolean;collection:Collection;entry:Entry;subject?:string;subjectColor?:string;seriesCount?:number;compact?:boolean;edit:()=>void;remove:()=>void;removeSeries?:()=>void;duplicate:()=>void;toggle:()=>void;pin:()=>void;remind:()=>void;scheduleReview?:()=>void;confirmReview?:()=>void;startFocus?:()=>void}){
 ...
 return <article className={'wb-record wb-record-'+collection+' '+(paper?'wb-sticky note-font-'+String(entry.font??'rounded'):'')+(done?' is-complete':'')+(needsReview?' needs-review-record':'')} style={paper?{'--note-paper':String(entry.color??'#fff2b4'),'--note-ink':String(entry.textColor??'#2f2942')} as CSSProperties:{'--subject-accent':subjectColor??String(entry.color??'#b47e75')} as CSSProperties}>
  <>{paper&&<span className="push-pin pin-0" aria-hidden="true"/>}</>{reviewBadge}<small>{subject??(entry.subjectId?'Removed subject':'General')} · {labels[collection]}{note&&entry.pinned?' · Pinned':''}</small><h3>{titleOf(entry)}</h3>{Boolean(entry.due||entry.date)&&<p>{dateLabel(String(entry.due??entry.date))}{collection==='calendarEvents'&&entry.time?' · '+timeRange(String(entry.time),entry.endTime as string|undefined):''}{done?' · Completed':''} {due&&<span className={'countdown-chip '+(daysUntil(due)<0?'is-overdue':'')}>{dueLabel}</span>}{plannedChip}{subtaskProgress&&<span className="subtask-chip">{subtaskProgress}</span>}{inSeries&&<span className="subtask-chip">1 of {seriesCount} dates</span>}</p>}
...
function Calendar({data,tasks,date,selectDate,create,render,renderClass,reschedule,setting,remove,navigate,parentMode,quickAddEvent}:{data:AppData;tasks:Task[];date:string;selectDate:(date:string)=>void;create:(key:Collection,date?:string)=>void;render:(key:Collection,entry:Entry,compact?:boolean)=>ReactNode;renderClass:(item:ClassOccurrence)=>ReactNode;reschedule:(key:Collection,entry:Entry,date:string)=>void;setting:<K extends keyof SettingsData>(key:K,value:SettingsData[K])=>Promise<boolean>;remove:(key:Collection,entry:Entry)=>void;navigate:(page:Page)=>void;parentMode:boolean;quickAddEvent:(date:string,payload:{title:string;kind:CalendarEventKind;kidId?:string;time?:string;endTime?:string})=>Promise<boolean>}){
 ...
 const dayFamilyList=(day:string)=>{
  const dayItems=[
   ...entries.filter(e=>e.date===day&&e.key!=='notes').map(e=>({id:e.key+':'+e.entry.id,title:titleOf(e.entry),time:e.key==='calendarEvents'?e.entry.time as string|undefined:e.key==='tasks'?e.entry.plannedTime as string|undefined:undefined,endTime:e.key==='calendarEvents'?e.entry.endTime as string|undefined:undefined,kidId:e.entry.kidId as string|undefined,removable:{key:e.key,entry:e.entry}})),
   ...scheduleClasses(day).map(c=>({id:'class:'+c.id,title:c.block.label,time:c.displayStart??c.block.start,endTime:c.displayEnd??c.block.end,kidId:c.block.kidId,removable:undefined as {key:Collection;entry:Entry}|undefined})),
  ]
 ...
 const familyDayList=(day:string)=>{
  ...
  return <ul className="family-day-agenda-list">{items.map(item=>{const kid=kidOf(item.kidId);return <li key={item.id} style={kid?{'--kid-color':kid.color} as CSSProperties:undefined}>{kid&&<i className="family-kid-swatch" aria-hidden="true"/>}{kid&&<strong>{(kid.emoji?kid.emoji+' ':'')+kid.name+': '}</strong>}<span>{item.title}</span>{item.time&&<small> · {timeRange(item.time,item.endTime)}</small>}{item.removable&&<button type="button" className="planner-due-remove" aria-label={'Remove '+item.title} onClick={()=>remove(item.removable!.key,item.removable!.entry)}>✕</button>}</li>})}{!items.length&&<li className="wb-muted">Nothing scheduled.</li>}</ul>
 }
