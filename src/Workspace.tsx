...
function Workspace({store}:{store:Store}){
 ...
 const quickAddFamilyEvent=async(date:string,payload:{title:string;kind:CalendarEventKind;kidId?:string;time?:string}):Promise<boolean>=>{
  try{
   const saved=await save(d=>({...d,calendarEvents:[...d.calendarEvents,{id:uid('event'),profileId:profile.id,date,title:payload.title,kind:payload.kind,kidId:payload.kidId,time:payload.time,notes:'',done:false}]}))
...
function RecordCard({cozy,collection,entry,subject,subjectColor,seriesCount=0,compact=false,edit,remove,removeSeries,duplicate,toggle,pin,remind,scheduleReview,confirmReview,startFocus}:{cozy:boolean;collection:Collection;entry:Entry;subject?:string;subjectColor?:string;seriesCount?:number;compact?:boolean;edit:()=>void;remove:()=>void;removeSeries?:()=>void;duplicate:()=>void;toggle:()=>void;pin:()=>void;remind:()=>void;scheduleReview?:()=>void;confirmReview?:()=>void;startFocus?:()=>void}){
 ...
 return <article className={'wb-record wb-record-'+collection+' '+(paper?'wb-sticky note-font-'+String(entry.font??'rounded'):'')+(done?' is-complete':'')+(needsReview?' needs-review-record':'')} style={paper?{'--note-paper':String(entry.color??'#fff2b4'),'--note-ink':String(entry.textColor??'#2f2942')} as CSSProperties:{'--subject-accent':subjectColor??String(entry.color??'#b47e75')} as CSSProperties}>
  <>{paper&&<span className="push-pin pin-0" aria-hidden="true"/>}</>{reviewBadge}<small>{subject??(entry.subjectId?'Removed subject':'General')} · {labels[collection]}{note&&entry.pinned?' · Pinned':''}</small><h3>{titleOf(entry)}</h3>{Boolean(entry.due||entry.date)&&<p>{dateLabel(String(entry.due??entry.date))}{collection==='calendarEvents'&&entry.time?' · '+classTime(String(entry.time)):''}{done?' · Completed':''} {due&&<span className={'countdown-chip '+(daysUntil(due)<0?'is-overdue':'')}>{dueLabel}</span>}{plannedChip}{subtaskProgress&&<span className="subtask-chip">{subtaskProgress}</span>}{inSeries&&<span className="subtask-chip">1 of {seriesCount} dates</span>}</p>}
...
function Calendar({data,tasks,date,selectDate,create,render,renderClass,reschedule,setting,remove,navigate,parentMode,quickAddEvent}:{data:AppData;tasks:Task[];date:string;selectDate:(date:string)=>void;create:(key:Collection,date?:string)=>void;render:(key:Collection,entry:Entry,compact?:boolean)=>ReactNode;renderClass:(item:ClassOccurrence)=>ReactNode;reschedule:(key:Collection,entry:Entry,date:string)=>void;setting:<K extends keyof SettingsData>(key:K,value:SettingsData[K])=>Promise<boolean>;remove:(key:Collection,entry:Entry)=>void;navigate:(page:Page)=>void;parentMode:boolean;quickAddEvent:(date:string,payload:{title:string;kind:CalendarEventKind;kidId?:string;time?:string})=>Promise<boolean>}){
 ...
 return <section className="wb-panel cozy-calendar-shell"><div className="wb-section-head"><h2>Your calendar</h2><div className="wb-toolbar"><button onClick={()=>{selectDate(localDate());setMonth(localDate().slice(0,7))}}>Today</button><div className="schedule-view-picker" role="group" aria-label="Schedule view"><label><input type="checkbox" checked={showAcademic} onChange={e=>void setting('scheduleShowAcademic',e.target.checked)}/> Academics</label><label><input type="checkbox" checked={showSports} onChange={e=>void setting('scheduleShowSports',e.target.checked)}/> Sports</label><label><input type="checkbox" checked={showAppointments} onChange={e=>void setting('scheduleShowAppointments',e.target.checked)}/> Appointments</label></div><label>Calendar view<select value={mode} onChange={e=>setMode(e.target.value)}><option value="agenda">Agenda</option><option value="month">Month</option></select></label></div></div>
  ...
  {mode==='agenda'&&<div className="wb-agenda-upcoming"><h4>Coming up</h4>{dayEntries.slice(1).map(({day,entries:dayEntriesForDate,classes:dayClasses},i)=>{const important=dayEntriesForDate.filter(dayHasPriority).length;return <details key={day} open={i===0} className={'wb-agenda-day'+(dropTarget===day?' drop-target':'')} {...dropOn(day)}><summary><h3>{dateLabel(day)}</h3><span className="wb-agenda-day-meta"><small>{dayClasses.length+dayEntriesForDate.length} item{dayClasses.length+dayEntriesForDate.length===1?'':'s'}{important?' · '+important+' important':''}</small>{parentMode&&<button type="button" className="quick-add-kid-event" onClick={e=>{e.preventDefault();e.stopPropagation();setQuickAdd(day)}}>＋ Add</button>}</span></summary>{schoolDayLabels(data,day).map(label=><p className="school-day-label" key={label}>{label}</p>)}{kids.length>0&&familyDayList(day)}{importantFirst(dayEntriesForDate.filter(e=>e.key==='exams'||!attachedToClass(e,dayClasses))).map(draggableEntry)}{dayClasses.map(renderClass)}{!dayClasses.length&&!dayEntriesForDate.length&&<p className="wb-muted">Nothing scheduled.</p>}</details>})}</div>}
...
function QuickFamilyAdd({kids,onCancel,onSave}:{kids:Kid[];onCancel:()=>void;onSave:(payload:{title:string;kind:CalendarEventKind;kidId?:string;time?:string})=>Promise<void>}){
 const [title,setTitle]=useState(''),[kidId,setKidId]=useState(kids[0]?.id??''),[kind,setKind]=useState<CalendarEventKind>('appointment'),[time,setTime]=useState(''),[busy,setBusy]=useState(false),[error,setError]=useState('')
 const submit=async(e:FormEvent)=>{
  e.preventDefault();if(busy||!title.trim())return
  setBusy(true);setError('')
  try{await onSave({title:title.trim(),kind,kidId:kidId||undefined,time:time||undefined})}catch(e){setError(e instanceof Error?e.message:'Could not save.')}finally{setBusy(false)}
 }
