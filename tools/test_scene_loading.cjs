...
test('Every manual boot requests exactly one phase, 217 textures, existing packaged assets',()=>{
  for(const phase of ['morning','afternoon','evening','night']){
    ...
    assert.equal(s.requests.length,217)
    ...
    assert.equal(s.scene.bootImageKeys.length,217)
  }
})
...
test('A -> B -> A never commits stale B; B pack has only 68 uncached textures',()=>{
  const s=setup();s.boot();s.request('night');s.request('afternoon');s.finish()
  assert.equal(s.batches[0].length,68)
