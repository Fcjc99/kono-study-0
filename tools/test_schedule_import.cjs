...
// A headed class table (as rebuilt from a PDF) becomes one weekly class per row, with its building, room
// and teacher kept, and a lecture and its discussion sharing one subject.
const classTable=suggestSchedule('Class Schedule\nClass Name\tTeacher\tDay\tTime\tBuilding\tRoom\nCorporate Finance\tOkafor, Ben\tTuesday & Thursday\t3:00 PM - 4:15 PM\t245 Beacon Street\t102\nEthics Seminar (Lecture)\tChen, Li\tTuesday & Thursday\t10:30 AM - 11:45 AM\tSouth Hall\t204\nEthics Seminar (Discussion)\tPark, Jo\tMonday\t12:00 - 12:50 PM\tWest Hall\t141N\nThe Monday discussion is listed separately.\t\t\t\t\t\nFinal exam 12/14/2026',start,end,'mdy')
assert.equal(classTable.filter(r=>r.kind==='class').length,3)
assert.equal(classTable[0].title,'Corporate Finance');assert.equal(classTable[0].weekdays.join(','),'2,4');assert.equal(classTable[0].start,'15:00');assert.equal(classTable[0].location,'245 Beacon Street 102 · Okafor, Ben')
assert.equal(classTable[2].weekdays.join(','),'1');assert.equal(classTable[2].start,'12:00');assert.equal(classTable[2].end,'12:50')
assert.ok(classTable.some(r=>r.kind==='exam'&&r.date==='2026-12-14'))
const tableImport=applyScheduleImport(base,id,classTable.map(r=>({...r,include:true})),start,end)
const tableSeason=tableImport.data.studySeasons.at(-1)
assert.equal(tableSeason.week.Tuesday.length,2);assert.equal(tableSeason.week.Monday[0].location,'West Hall 141N · Park, Jo')
assert.equal(tableImport.data.subjects.filter(s=>s.name==='Ethics Seminar').length,1)
console.log('PASS schedule import: a PDF class table becomes weekly classes with building, room and teacher.')
