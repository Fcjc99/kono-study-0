...
test('question lists and TSV import produce bounded, literal flashcards',()=>{
 const today='2026-09-18',f=load('src/store/flashcards.ts');const cards=f.parseFlashcards('Q one :: A one\r\nQ two\tA two',randomUUID,today);same(cards.map(c=>[c.question,c.answer]),[['Q one','A one'],['Q two','A two']]);assert.throws(()=>f.parseFlashcards('unstructured notes',randomUUID,today));assert.throws(()=>f.parseFlashcards('Q :: ',randomUUID,today));assert.throws(()=>f.parseFlashcards(Array(201).fill('Q :: A').join('\n'),randomUUID,today));cards[0]={...cards[0],interval:5,ease:2.4,dueDate:'2026-09-25'};same(f.reviewQueue({cards},today,true),[cards[1].id]);same(f.reviewQueue({cards},today),[cards[1].id])
});
test('decks survive export and two-device review merges with profile isolation',()=>{
 const today='2026-09-18',f=load('src/store/flashcards.ts'),base=model.createFreshData();base.flashcardDecks=[{id:'deck',profileId:base.activeProfileId,title:'Test',cards:f.parseFlashcards('First :: Answer\nSecond :: Answer',randomUUID,today)}];same(model.normalizeData(JSON.parse(JSON.stringify(base))),base);const left=structuredClone(base),right=structuredClone(base);left.flashcardDecks[0].cards[0]={...left.flashcardDecks[0].cards[0],interval:1,ease:2.4,dueDate:'2026-09-19'};right.flashcardDecks[0].cards[1]={...right.flashcardDecks[0].cards[1],interval:1,ease:2.4,dueDate:'2026-09-19'};const merged=mergeData(base,left,right);assert.equal(merged.conflicts.length,0);assert.equal(merged.data.flashcardDecks[0].cards.filter(c=>c.interval===0).length,0);base.flashcardDecks[0].profileId='foreign';assert.throws(()=>model.normalizeData(base));
});
test('opposing reviews on two devices surface a conflict even when one repeats the old grade',()=>{
 const today='2026-09-18',f=load('src/store/flashcards.ts'),base=model.createFreshData();base.flashcardDecks=[{id:'deck',profileId:base.activeProfileId,title:'Review',cards:f.parseFlashcards('Q :: A',randomUUID,today)}];const left=structuredClone(base),right=structuredClone(base);left.flashcardDecks[0].cards[0]={...left.flashcardDecks[0].cards[0],interval:1,ease:2.4,dueDate:'2026-09-19',reviewId:'review-left'};right.flashcardDecks[0].cards[0]={...right.flashcardDecks[0].cards[0],interval:0,ease:2.1,dueDate:today,reviewId:'review-right'};assert.ok(mergeData(base,left,right).conflicts.length)
});
